...
tf-idf is a metric of a term's importance in a document with regards to a set of 
training data (a corpus), and in the context of this project, it represents the the importance of a word in a certain
...
def compute_corpus_tf_idf(corpus: list, corpus_idf: dict) -> dict:
    ...
    for ele in corpus:
        ...
        # add the tf-idf value of each song to the dict, with the keys being
        # the ids of songs
...
def nearest_neighbor(
    ...
    score_table = {}
    for ele in corpus:
        # go trough each element in corpus, calculate the cosine similarity
        # of each song to the new song given
        # add the cosine similarity value to the dict, the key being the id
        # of the song that was used to compare
        score_table[ele.id] = cosine_similarity(compute_tf_idf(new_lyrics,
                                            corpus_idf), corpus_tf_idf[ele.id])
